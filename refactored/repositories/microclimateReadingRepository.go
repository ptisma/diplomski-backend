...
func (r *MicroclimateReadingRepository) GetMicroClimateReadings(microclimateID, locationID int, fromDate, toDate time.Time) ([]models.MicroclimateReading, error) {

	var err error
	microclimates := []models.MicroclimateReading{}
	queryStr := "microclimate_id = ? AND location_id = ? AND date <= ? AND date >= ?"

	err = r.DB.Debug().Preload("Location").Model(&models.MicroclimateReading{}).Preload("Microclimate").Where(queryStr, microclimateID, locationID, toDate.Format("2006-01-02"), fromDate.Format("2006-01-02")).Group("microclimate_id,location_id,date").Order("date").Find(&microclimates).Error
	if err != nil {
...
func (r *MicroclimateReadingRepository) GetLatestMicroClimateReading(locationID int) (models.MicroclimateReading, error) {

	var err error
	microclimateReading := models.MicroclimateReading{}
	queryStr := "location_id = ?"

	err = r.DB.Debug().Preload("Location").Model(&models.MicroclimateReading{}).Preload("Microclimate").Where(queryStr, locationID).Order("date desc").First(&microclimateReading).Error
